...
export function buildAnalyse(input: AnalyseInput): AnalyseResult {
  ...
  if (input.clientsEnContentieux.nombre > 0) {
    pointsVigilance.push(
      `${input.clientsEnContentieux.nombre} client${input.clientsEnContentieux.nombre > 1 ? 's' : ''} actuellement en litige (palier ≥ 7), représentant ${input.clientsEnContentieux.montant.toLocaleString('fr-FR')} FCFA d'encours.`,
    );
    recommandations.push(
      `Statuer sur les dossiers en litige (relance judiciaire, échéancier négocié ou provision) plutôt que de les laisser en l'état — ${input.clientsEnContentieux.montant.toLocaleString('fr-FR')} FCFA d'encours immobilisé.`,
    );
  }
